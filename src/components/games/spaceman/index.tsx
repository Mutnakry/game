...
import { CosmicBackground } from "@/components/games/spaceman/cosmic-background"
import { SpacemanDisplay } from "./spaceman-display"
import { GameControls } from "@/components/games/spaceman/game-controls"
import { useMultiplierData } from "@/components/games/spaceman/hooks/use-multiplier-data"
import { useCrashData } from "@/components/games/spaceman/hooks/use-crash-data"
import { useGameLogic } from "@/components/games/spaceman/hooks/use-game-logic"
...
export default function SpaceMan() {
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-purple-900/10 to-violet-900/10 backdrop-blur-[2px]"></div>
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center justify-center px-4 py-8">
        ...
        <div className="flex flex-col items-center justify-center text-center mb-4">
          <img src="/spaceman.webp" className="h-28 mx-auto" alt="Space Man" />
        </div>
        ...
        {isLoadingData ? (
          <div className="w-full max-w-sm bg-black/30 backdrop-blur-md rounded-xl p-6 border border-white/10 shadow-xl mb-6">
            <div className="flex items-center justify-center space-x-2">
              ...
              <div className="w-3 h-3 rounded-full bg-violet-500 animate-pulse delay-150"></div>
              <div className="w-3 h-3 rounded-full bg-fuchsia-500 animate-pulse delay-300"></div>
              <span className="text-white ml-2">Loading game data...</span>
            </div>
          </div>
        ) : dataError ? (
          <div className="w-full max-w-sm bg-purple-900/30 backdrop-blur-md rounded-xl p-4 border border-purple-500/30 shadow-xl mb-6">
            <div className="text-purple-300 text-center">
              <p className="font-bold">Error loading game data</p>
              <p className="text-sm">{dataError}</p>
              <p className="text-xs mt-1 text-purple-400">Using default values</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="relative w-full flex flex-col items-center justify-center mb-8">
          <SpacemanDisplay
            ...
            countingMultiplier={countingMultiplier}
      ...
      <div className="relative z-10 w-full text-center text-gray-500 text-xs mt-8 mb-4">
        <p>© {new Date().getFullYear()} Space Man Game. All rights reserved.</p>
      </div>
    </div>
  )
}
